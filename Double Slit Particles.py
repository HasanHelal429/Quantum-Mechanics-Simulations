...
for i in range(tIteration + 1):
    ...
    for j in range(numP):
        ...
        if ((4.9 < p1[j].x[i]) & (p1[j].x[i] < 5.1) 
            & (((p1[j].y[i] > 6) |  (p1[j].y[i] < 4)) | ((p1[j].y[i] > 4.5) & (p1[j].y[i] < 5.5)))):
            ...
            for b in t:
              ...
              ~ (((p1[j].y[i] > 6) |  (p1[j].y[i] < 4)) | ((p1[j].y[i] > 4.5) & (p1[j].y[i] < 5.5))) & ~collided[j]):
...
wall_bottom = Rectangle((4.9,0), 0.2, 4,      color=slitcolor, zorder=50, alpha=slitalpha) # (x0, y0), width, height
wall_middle = Rectangle((4.9, 4.5), 0.2, 1, color=slitcolor, zorder=50, alpha=slitalpha)
wall_top    = Rectangle((4.9,6), 0.2, 4,      color=slitcolor, zorder=50, alpha=slitalpha)
...
ani.save(filename="/Users/hasan/Python Animations/double_slit_particles.gif", writer="pillow", fps=50)
